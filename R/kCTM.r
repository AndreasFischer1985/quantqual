...
kCTM <- function (data = NULL, d = NULL, K = c(2:10), method = "VEM", 
    ...
    plot = T, ...) 
{
    ...
    de = numeric(0)
    for (k in K) {
        l = topicmodels::CTM(d, k, method = method, control = control, 
            ...)
        co = c(co, mean(coherenceLDA(l, d)))
        ex = c(ex, mean(exclusivityLDA(l, d)))
        ar = c(ar, Arun(l, d))
        de = c(de, Deveaud(l))
