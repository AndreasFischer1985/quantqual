...
#' 
#' Correlates code frequencies across documents with a numeric vector.
#' @param corpus Character vector containing one document per element.
#' @details Correlates code frequencies across documents with a numeric vector.
    ...
    if (mostFrequent > 0) {
        ...
        if (sum(colSums(t, na.rm = T) > i, na.rm = T) != dim(t)[2]) {
            message(paste0("Focus on interactions of ", sum(colSums(t, 
                na.rm = T) > i, na.rm = T), " most relevant codes (out of ", 
                dim(t)[2], "); i=", i))
